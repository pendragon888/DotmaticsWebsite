...
setup.describe('Smoke test web application', async () => {
    setup('Verify each page can be accessed', async ({ landingPage, aboutCareersPage, jobsPage, contactUsPage, page }) => {
        ...
        await expect(page).toHaveTitle('Careers in Science & Technology | Work at Dotmatics')
        await expect (aboutCareersPage.exploreAvailablePositionsLink).toBeVisible()
        await jobsPage.goto()
        await expect(page).toHaveTitle('Open Positions Dotmatics')
        await expect (jobsPage.bannerBody).toBeVisible()
